...
用户随后文字确认云端CRUD成功，单篇爬取、精确查询、详情及外部来源有5张截图并确认正常。这些是人工验收，仍无完整浏览器自动化证据；批量导入、动画推进等其他路径待验收。Free实例的SQLite为临时磁盘，更新/重启可能重置，不能宣称云端永久保存数据。下载CSV可保留展示数据，但不等于完整数据库备份。Compose具名卷方案为另一种部署方式，本次Render没有使用该卷。
