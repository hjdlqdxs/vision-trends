...
用户随后文字确认云端CRUD成功，单篇爬取、精确查询、详情及外部来源有5张截图并确认正常。这些是人工验收，仍无完整浏览器自动化证据；随后批量TXT导入截图显示识别2个标题、成功2/2；热度走势播放、暂停、滑块和指标切换已获用户实际操作确认，3张状态截图已归档；图谱节点筛选与返回也已确认正常，2张截图已归档；年度演变播放、滑块及论文库联动也已确认正常，2张截图已归档；CSV、部分失败等其他路径待验收。Free实例的SQLite为临时磁盘，更新/重启可能重置，不能宣称云端永久保存数据。下载CSV可保留展示数据，但不等于完整数据库备份。Compose具名卷方案为另一种部署方式，本次Render没有使用该卷。

## 单篇采集追加验收

用户已启用管理权限并确认增删改成功。2026-09-23单篇采集Segment Anything（ICCV2023）截图显示新增成功、摘要已获取及2个标签；精确查询命中1篇（编号190），PDF和CVF来源正常打开。5张原始截图已加入evidence与博客。189为初始样本数，不再作为运行中数据库必须不变的断言。
