...
图2：节点代表研究方向，连线表示同篇论文共现。此处为静态统计导出图；真实网页中的节点联动截图见下文。

### 5.2.1 公网热度走势人工验收

2026-09-23，用户实际操作并确认“播放、暂停、拖动、切换指标都正常”。以下三张原始截图展示同一研究方向3D vision在不同年份和统计指标下的页面状态。播放与暂停的通过依据是用户操作反馈，静态截图用于展示结果。

![样本占比截至2025年](evidence/trend-percent-2025.png)

![拖动年份滑块至2023年](evidence/trend-percent-2023.png)

![切换为论文数量](evidence/trend-count-2025.png)

截至2023年时仅显示该年及之前的数据点；数量模式显示篇数，比例模式显示百分比。本项验收覆盖热度走势，年度演变及关键词图谱另有独立验收记录。

### 5.2.2 关键词图谱联动验收

2026-09-23，用户在研究总览点击关键词共现图谱中的节点，成功进入论文资料库并保留 `3D vision` 筛选条件；结果页显示匹配论文，页面提供“返回研究总览”入口，用户确认跳转和返回均正常。

![研究总览与关键词图谱](evidence/graph-overview.png)

![图谱节点进入关键词筛选结果](evidence/graph-keyword-filter.png)

这两张截图展示了真实网页中的图谱及带关键词的论文列表，跳转与返回正常另由用户确认；图谱连线的含义仍按同篇论文共现统计解释。

### 5.2.3 年度演变联动验收

2026-09-23，用户实际打开“年度演变”，确认播放和年份滑块正常；点击年度热门方向后进入论文资料库，保留 `3D vision` 与 `2025` 筛选条件，页面显示12篇结果，返回年度演变入口可见。

![年度演变2024榜单](evidence/annual-evolution-2024.png)

![年度方向进入论文筛选](evidence/annual-evolution-filter-result.png)

本项截图记录年度榜单状态和年度方向到论文库的联动；播放正常依据用户实际操作反馈，静态截图用于展示结果。
...
**剩余界面证据**：已有单篇与批量TXT采集、精确查询、图谱联动、热度走势和年度演变截图；仍需补CRUD过程、未命中在线回退及手机视口等展示。仅统计图不足以证明全部功能。`scripts/capture-browser.ps1` 可自动准备14类界面素材，但本会话浏览器IPC受限，尚未运行成功；也可以真实手动截图或录屏。
...
自动浏览器截图受Windows IPC限制，不能把HTTP测试或统计图视为真实浏览器验收。用户已手动浏览本地网站并确认返回导航可用；Figma插件执行有成功截图，原型返回路径已由用户确认，分享链接已由用户通过无痕窗口验证可访问。现已在Render部署；用户无痕窗口截图证明公网走势页可见。AI只读检查健康、论文列表、趋势和权限配置4个接口均返回200，论文总数189，详见 `cloud-readonly-check.json`；之后用户确认云端CRUD成功，并提供单篇采集、精确查询、详情与来源的5张截图；随后批量TXT导入截图显示2/2成功；热度走势播放、暂停、拖动及指标切换已有用户确认和3张状态截图；图谱节点到关键词筛选及返回也已通过2张截图。未命中联网查询等场景待验证；年度演变播放、滑块及论文库联动已有人工验收截图。
...
- 已按老师更新通知免费部署到Render，给出真实公网URL和访问截图；云端CRUD及单篇采集已有用户验证，批量TXT导入已有2/2成功截图，热度走势四项交互、图谱联动与返回均已通过人工验收；未命中联网查询等场景待补；年度演变已完成。
