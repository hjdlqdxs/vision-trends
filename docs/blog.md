...
自动浏览器截图受Windows IPC限制，不能把HTTP测试或统计图视为真实浏览器验收。用户已手动浏览本地网站并确认返回导航可用；Figma插件执行有成功截图，原型返回路径已由用户确认，分享链接已由用户通过无痕窗口验证可访问。现已在Render部署；用户无痕窗口截图证明公网走势页可见。AI只读检查健康、论文列表、趋势和权限配置4个接口均返回200，论文总数189，详见 `cloud-readonly-check.json`；之后用户确认云端CRUD成功，并提供单篇采集、精确查询、详情与来源的5张截图；批量导入等剩余场景待验证。
...
- 已按老师更新通知免费部署到Render，给出真实公网URL和访问截图；云端CRUD及单篇采集已有用户验证，批量导入等剩余场景待补。
